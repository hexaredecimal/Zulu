...
import com.zulu.ast.InstanceFieldAccess;
...
import com.zulu.runtime.Externals;
...
import com.zulu.optimizations.TableEmulator;
import com.zulu.optimizations.VariableInfo;
import com.zulu.runtime.TypeTable;
import com.zulu.units.UnitBase;
import com.zulu.units.Units;
import com.zulu.utils.FileUtils;
import com.zulu.utils.Handler;
import com.zulu.ast.ImportAst;
import com.zulu.ast.ListIndexAST;
import com.zulu.ast.MapIndexAST;
...
public final class Parser implements Serializable {
	...
	private AST conditional() {
		...
		if (binaryOperators.containsKey(get(0).getType())) {
			...
			args.add(result);
			args.add(right);
			return new CallAST(new ConstantAST(new ZuluFunction(binaryOperators.get(type))), args, line(), currentLine());
		}
		return result;
	}
	...
	private AST unary() {
		...
		if (unaryOperators.containsKey(get(0).getType())) {
			...
			ArrayList<AST> args = new ArrayList<>();
			args.add(right);
			return new CallAST(new ConstantAST(new ZuluFunction(unaryOperators.get(type))), args, line(), currentLine());
		}
	...
	private AST primary() {
		...
		if (match(TokenType.NUMBER)) {
			try {
				return new ConstantAST(new ZuluNumber(Double.parseDouble(current.getText())));
			} catch (NumberFormatException ex) {
				return new ConstantAST(new ZuluNumber(Integer.parseInt(current.getText(), 32)));
			}
		}
		...
		if (match(TokenType.ATOM)) {
			return new ConstantAST(new ZuluAtom(current.getText()));
		}
	...
	private CaseAST match() {
		...
		while (!match(TokenType.END)) {
			...
			if (match(TokenType.NUMBER)) {
				// case 0.5:
				pattern = new CaseAST.ConstantPattern(new ZuluNumber(Double.parseDouble(current.getText()))
				);
			} else if (match(TokenType.STRING)) {
				// case "text":
				pattern = new CaseAST.ConstantPattern(
					new ZuluString(current.getText())
				);
			} else if (match(TokenType.VAR)) {
				// case value:
				pattern = new CaseAST.VariablePattern(current.getText());
			} else if (match(TokenType.ATOM)) {
				pattern = new CaseAST.ConstantPattern(new ZuluAtom(current.getText()));
	...
	private AST expandCall() {
		AST target = remote();
		return new CallAST(new RemoteAST(new ConstantAST(new ZuluAtom(module)), target, line(), currentLine()), arguments(), line(), currentLine());
	}
	...
	private AST buildCall(String module, String method, ArrayList<AST> args) {
		return new CallAST(new RemoteAST(new ConstantAST(new ZuluAtom(module)), new ConstantAST(new ZuluAtom(method)), line(), currentLine()), args, line(), currentLine());
	}

	private void emitVariable(String name) {
		emulator.set(name, new VariableInfo(new ZuluNull(), 0));
	}
